...
$fieldIDs = explode(',', $_REQUEST["field_id"]);
$params = array();
...
for ($k = 0; $k < count($fieldIDs); $k++) {
    ...
    $stmt = $db->prepare($sql);
    $stmt->execute(array('field_id'=>$fieldIDs[$k]));
    ...
    $sqlQuery .= "SELECT ". $fieldName ." as value FROM " . $fromString;
    if(!empty($filters)) {
        $sqlQuery .= ' where '.implode(' and ', $filters);
    }
}

$sqlQuery = "SELECT DISTINCT value FROM (" . $sqlQuery . ") AS XS order by value limit 50";
